...
private:
	...
	//SurfaceFlinger运行于SystemServer这一系统进程中，需要UI界面显示的应用程序则通过binder服务与它进行跨进程通信。
	//在音频系统的学习中，每一个AudioTrack在AudioFlinger中都可以找到一个对应的Track实现。这种设计方式同样适用于显
	//示系统，即任何有UI界面的程序都在SurfaceFlinger中有且仅有一个Client实例。
	//应用程序与SurfaceFlinger间的接口是ISurfaceComposerClient
...
int main(int argc, char **argv) {
	...
	//负责打开Binder驱动，建立线程池，让其进程里面的所有线程都能通过Binder通信 http://blog.csdn.net/pi9nc/article/details/9749325
	//每个线程都有一个IPCThreadState实例登记在Linux线程的上下文附属数据中，主要负责Binder的读取，写入和请求处理框架
    ProcessState::self()->startThreadPool();

	//基类DataSource提供了一些分离器
		...
		case 'c':
            {
                ...
                if (colonPos == NULL) {
                    ...
                    connectToPort = WifiDisplaySource::kWifiDisplayDefaultPort;   //kWifiDisplayDefaultPort = 7236;
                } else {
            ...
            case 'l':
            {
                ...
                if (colonPos == NULL) {
                    listenOnAddr = optarg;
                    listenOnPort = WifiDisplaySource::kWifiDisplayDefaultPort;   //kWifiDisplayDefaultPort = 7236;
                } else {
	...
	//strong pointer，而wp则是weak pointer的意思
